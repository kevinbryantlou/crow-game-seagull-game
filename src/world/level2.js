 ...
 *
 * The back of the Vantage Hotel, from the bins to the water tank. Level 1 is a
 * block: one floor, three districts, money density falling left to right. This
 * is the same idea stood on its end. There are four decks — yard, fire escape,
 * terrace, roof — and money density rises as you climb, so the value ladder and
 * the altitude ladder are the same ladder.
 *
...
export function buildLevel() {
  ...
  const { solid, perch, addPlanter, addLamp, addBin, addTable, addSkyline, makeNest, addPool } = kit;
  ...
  // The plunge pool. Same object as the plaza fountain, five metres in the air:
  // the dive is unchanged and the sum around it is not, because you climb out of
  // this one wet with the whole roof still above you.
  const PLUNGE = addPool(POOL.x, POOL.z, POOL.r, DECK.terrace, {
    stone: PAL.stone, coping: PAL.stone, lining: PAL.awning, tag: 'plunge-rim',
  });
  const FOUNTAIN = PLUNGE.spec;
  root.userData.fountainWater = PLUNGE.water;
  night.add(PLUNGE.water, PAL.water, { peak: 0.075, warm: 5.0, delay: 1.8 });
  perch(POOL.x, FOUNTAIN.rim, POOL.z - POOL.r);
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // THE LOFT — somebody keeps pigeons up here, which is why there is grain
  // ══════════════════════════════════════════════════════════════════════════
  {
    const g = new THREE.Group();
    g.add(at(box(2.6, 1.7, 2.0, PAL.bark, { up: PAL.bark, down: PAL.barkShade }), 0, DECK.roof + 0.85, 0));
    g.add(at(box(2.9, 0.14, 2.3, PAL.barkShade, { up: PAL.bark, down: PAL.shade }), 0, DECK.roof + 1.78, 0));
    // The landing board, which is also a perch, which is also where the tin is.
    g.add(at(box(1.6, 0.09, 0.8, PAL.barkShade, { up: PAL.bark }), 0, DECK.roof + 1.0, 1.35));
    for (const hx of [-0.5, 0.5]) {
      g.add(at(box(0.34, 0.4, 0.08, PAL.shade, { shadow: false }), hx, DECK.roof + 1.25, 1.01));
    }
    const bulb = at(ico(0.12, 0, PAL.goldLit, { shadow: false }), 0, DECK.roof + 1.62, 1.2);
    bulb.material = mat(PAL.goldLit);
    g.add(bulb);
    night.add(bulb, PAL.goldLit, { peak: 0.85, warm: 1.4, delay: 2.6, flicker: true });
    night.addPool(root, LOFT.x, LOFT.z + 1.4, 3.8,
      { profile: 'stall', peak: 0.7, warm: 1.4, delay: 2.6, flicker: true, y: DECK.roof });
    g.position.set(LOFT.x, 0, LOFT.z);
    root.add(g);
    solid(LOFT.x, LOFT.z, 2.6, 2.0, DECK.roof + 1.7);
    solid(LOFT.x, LOFT.z + 1.35, 1.6, 0.8, DECK.roof + 1.05, DECK.roof + 0.96);
    perch(LOFT.x, DECK.roof + 1.7, LOFT.z);
    perch(LOFT.x, DECK.roof + 1.05, LOFT.z + 1.35);
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // THE YARD — bins, crates, a van, and the cheapest money on the block
  // ══════════════════════════════════════════════════════════════════════════
  // Two lamps, not one. The yard is the bottom of a light well and it goes
  // darker than anywhere on level 1; one pole left the whole eastern end,
  // where the van and the crates are, with nothing at all.
  addLamp(14, 12.5);
  addBin(2, 11.5, PAL.canopy);
  addBin(3.6, 13.2, PAL.steel);
  addBin(-15, 11, PAL.canopy);
  addLamp(-2, 11);
  ...
  const addCrates = (x, z, n = 3) => {
    ...
    for (let i = 0; i < n; i++) {
      const s = 1.3 - i * 0.09;
      // Pale crates. The dark stack was one of the few things left in the darkest
      // 5% of the dusk frame once the wall stopped being.
      const c = box(s, 0.62, s, i % 2 ? PAL.pavingMid : PAL.bark, { up: PAL.paving, down: PAL.shade });
  ...
  const dockCrateTop = addCrates(-21, 8, 3);
  addCrates(16, 6.5, 2);
  addCrates(-2, 12.4, 3);
  const doorCrateTop = addCrates(-8.4, 3.4, 1);

  // The loading dock — a raised lip, and the first thing to stand on out of the
  // yard. Everything about this block is a staircase.
  {
    const d = box(8, 1.15, 4, PAL.stoneMid, { up: PAL.stone, down: PAL.shade });
    d.position.set(-18, 0.575, 5.5);
    root.add(d);
    solid(-18, 5.5, 8, 4, 1.15);
    perch(-18, 1.15, 5.5);
  }

  // The delivery van. Near side, so it fades rather than hiding the yard.
  {
    const g = new THREE.Group();
    ...
    g.add(stripe);
    const glass = box(0.16, 0.7, 1.7, PAL.water, { shadow: false });
    ...
    // a concave pocket it cannot commit its way out of. A van is one obstacle;
    // it has to have one outline.
    solid(9, 10, 4.8, 2.2, 2.55);
    solid(12.6, 10, 2.4, 2.2, 1.85);
    perch(9, 2.55, 10);
  }

  // A drain, and the grate the yard's quarters have collected in.
  {
    const grate = box(1.1, 0.05, 1.1, PAL.steelDark, { up: PAL.steel, down: PAL.shade });
    grate.position.set(-4.5, 0.026, 7.5);
    root.add(grate);
  }
...
function pickupPlacements({ FOUNTAIN, tableTops, dockCrateTop, doorCrateTop }) {
  ...
  // — The yard: free, plentiful, and priced by the climb rather than by a guard —
  for (const [x, z] of [
    [-12, 6.5], [-6, 9.5], [1, 6.2], [7, 13], [13, 11.5], [-17, 12],
    [-9.5, 7.8], [4.5, 9], [18, 9.5], [-21, 12.5],
  ]) add('penny', 0.01, x, Y + 0.06, z);
  for (const [x, z] of [[-13.5, 9.6], [0, 12.6], [11, 5.4], [20, 6]]) {
  ...
  add('nickel', 0.05, -19, 1.15 + 0.06, 5.5);
  for (const [x, z] of [[-7.5, 12], [5.5, 4.6], [15, 13], [-23, 7]]) {
    add('dime', 0.10, x, Y + 0.06, z);
  }
  // In the drain grate, which is the yard's one thing worth crossing it for.
  add('quarter', 0.25, -4.5, Y + 0.09, 7.2);
  add('quarter', 0.25, -4.2, Y + 0.09, 7.9);

  // The porter's open crate, and the chef's tin by the kitchen door.
  add('coins', 1.20, -21, dockCrateTop + 0.04, 8, { owner: 'porter' });
  add('bill5', 5.00, -8.4, doorCrateTop + 0.04, 3.4, { owner: 'chef', label: 'THE TIN' });

  // — The fire escape: the reward for taking the authored route —
  ...
  // Wishing coins, five metres in the air.
  // Radii capped at 1.85. Further out than that and a coin's sightline to the
  // camera clips the inside of the basin wall on the way up — the rim is 0.28
  // above the coins and the camera climbs at 38°, which buys 0.36 of horizontal
  // travel to clear it. On a ground-level fountain with a 5.2m basin there was
  // room not to think about this.
  for (const [deg, r] of [[30, 1.5], [95, 1.75], [160, 1.4], [215, 1.85], [285, 1.55], [340, 1.8]]) {
  ...
  // — The roof: the loft's feed tin, nobody's and everybody's —
  add('coins', 2.00, LOFT.x + 0.02, R + 1.10, LOFT.z + 1.35, { owner: 'cleaner', label: 'THE FEED TIN' });
  ...
  add('shiny', 0, -13.5, Y + 0.07, 5.2, { shinyKind: 'foil' });
  add('shiny', 0, -4.1, DECK.escape + 0.07, 2.15, { shinyKind: 'screw' });
  add('shiny', 0, FOUNTAIN.x, FOUNTAIN.rim - 0.28, FOUNTAIN.z, { inWater: true, shinyKind: 'clip' });
...
function humanPlacements() {
  return [
    ...
    },
    {
      ...
      pos: [-4.8, DECK.terrace, -0.2], home: [-4.8, DECK.terrace, -0.2],
      patrol: [[-4.8, -0.2], [2.0, -4.8], [6.0, 0.0], [-10.6, -0.3]],
    ...
    },
    {
      ...
      // In the doorway, facing out. He is not watching the yard, he is watching
      // the middle distance, which is why his cone is narrow and his tin is not.
      pos: [-10, DECK.yard, 3.0], home: [-10, DECK.yard, 3.0],
    ...
    },
    {
      ...
      pos: [-6, DECK.yard, 6], home: [-6, DECK.yard, 6],
      patrol: [[-19, 9.5], [-8, 9], [4, 12], [14, 8], [18, 4], [6, 5], [-6, 6]],
      speed: 1.35, chaseSpeed: 3.4, viewDist: 3.2, viewCos: 0.8, guardRadius: 1.8, alertness: 0.3,
      oblivious: true,
    },
    {
      id: 'cleaner', name: 'the window cleaner', cloth: 4, skin: 1, hair: 0,
      pos: [-11.6, DECK.roof, -7.5], home: [-11.6, DECK.roof, -7.5],
      patrol: null, speed: 1.1, chaseSpeed: 3.6, viewDist: 8, viewCos: 0.3, guardRadius: 3.0, alertness: 0.9,
      faces: [1, 0],
    },
    {
      id: 'kid', name: 'the kid on the fire escape', cloth: 2, skin: 2, hair: 2,
      pos: [-5.0, DECK.escape, 2.6], home: [-5.0, DECK.escape, 2.6],
...
function gullPlacements() {
  return [
    ...
    { x: -13.5, z: -10.2, y: DECK.roof },
    { x: -20.5, z: -6.6, y: DECK.roof },
  ];
}

/** The loft's own birds, and they stay in the yard where the bins are. */
function pigeonPlacements() {
  return [
    { x: -3, z: 10 }, { x: 1.5, z: 12.5 }, { x: -14, z: 9.5 },
    { x: 6, z: 8 }, { x: -18, z: 12 },
  ];
}
